...
integer :: l, error
integer :: argCount
logical :: damp_flag, kappa_flag, nf_flag
character(len=10) :: arg
double precision,parameter :: Eh2eV=27.21138505d0
double precision,parameter :: Pi=4.d0*atan(1.d0)
double precision,parameter :: c=137.d0
double precision :: kappa
double precision :: pf, specmax
double precision :: damp, time, ts, omega
...
double complex,allocatable,dimension(:) :: fft
...
if (argCount.ne.2) then
 write(*,*) 'Use: rttddft_spec [Damping time constant] [Electric field strength]'
 stop
end if
...
pf=(4.d0*Pi)/(3.d0*c*kappa)
open(unit=10,file='dip.dat',status='old',iostat=error)
if (error.ne.0) then
 write(*,*) 'Problem opening dipole file'
 stop
end if
open(unit=30,file='nf.dat',status='old',iostat=error)
if (error.ne.0) nf_flag=.false.
n=0
do
...
do i=2,l
 omega=2.d0*Pi*(dble(i-1)/(dble(pn)*ts))
 if (Eh2eV*omega.gt.specmax) exit
 write(20,'(f12.6,4es19.10)') Eh2eV*omega,-omega*pf*sum(r(:,i)),-omega*pf*r(:,i)
end do
close(20)
deallocate(r)
end program rttddft_spec
